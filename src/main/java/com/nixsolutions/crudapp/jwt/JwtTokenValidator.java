...
import com.nixsolutions.crudapp.exception.InvalidTokenException;
...
public class JwtTokenValidator {
    ...
    private JsonObject decodeTokenPayloadToJsonObject(DecodedJWT decodedJWT) {
        try {
            ...
            return new Gson().fromJson(
                    new String(Base64.getDecoder().decode(payloadAsString),
    ...
    private Instant extractExpirationDate(JsonObject payloadAsJson) {
        try {
            ...
            throw new InvalidTokenException(
                    "There is no 'exp' claim in the token payload");
        }
    }

    private boolean hasTokenRealmRolesClaim(JsonObject payloadAsJson) {
        try {
            return payloadAsJson.getAsJsonObject("realm_access")
                    .getAsJsonArray("roles").size() > 0;
        } catch (NullPointerException ex) {
            return false;
        }
    }
    ...
    private String subStringBearer(String authorizationHeader) {
        try {
            ...
            throw new InvalidTokenException(
                    "There is no AccessToken in a request header");
        }
    }
}