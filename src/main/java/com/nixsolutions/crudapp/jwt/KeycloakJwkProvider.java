...
public class KeycloakJwkProvider implements JwkProvider {
    ...
    public Jwk get(String keyId) throws JwkException {
        ...
        throw new SigningKeyNotFoundException(
                "No key found in " + uri.toString() + " with kid " + keyId,
                null);
    }

    private List<Jwk> getAll() throws SigningKeyNotFoundException {
        List<Jwk> jwks = Lists.newArrayList();
        final List<Map<String, Object>> keys = (List<Map<String, Object>>) getJwks().get(
                "keys");

        if (keys == null || keys.isEmpty()) {
            throw new SigningKeyNotFoundException(
                    "No keys found in " + uri.toString(), null);
        }
        ...
        try {
            ...
            throw new SigningKeyNotFoundException(
                    "Failed to parse jwk from json", e);
        }
        return jwks;
    }
    ...
    private Map<String, Object> getJwks() throws SigningKeyNotFoundException {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder().uri(this.uri)
                    .headers("Accept", "application/json").GET().build();
            ...
            throw new NetworkException(
                    "Cannot obtain jwks from url " + uri.toString(), e);
        }
    }
}